...
- Persistent scan storage in Docker volume `scan_data` and a parametric 3D floor-plan view in Haven.

The app has no third-party iOS packages. RoomPlan, SwiftUI, and URLSession are supplied by Apple.
...
- The updated Haven Docker stack running from this branch.
...
```bash
git fetch origin
git checkout codex/ios-lidar-scanner
git pull --ff-only
...
5. Move slowly around the room. RoomPlan overlays geometry while Haven's bottom tray updates every structural count and object category live.
6. Tap **Finish this room**.
7. Name it. Choose **Scan another room** or **Finish home and return**.
8. The app uploads the semantic model and reopens Haven with the completed scan ID.
9. Haven shows the RoomPlan model, room and detection counts, and smart candidates such as televisions or lamps.
